...
@router.get("/{tienda_id}/", response=List[ProductoSchema])
@paginate
...
@router.post("/", response=ProductoSchema)
def create_producto(request, producto_in: ProductoInSchema):
    """
...
@router.patch("/{producto_id}/", response=ProductoSchema)
def update_producto(request, producto_id: int, producto_in: ProductoInSchema):
    """
...
@router.delete("/{producto_id}/", response={204: None})
